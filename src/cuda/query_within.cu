...
__global__ void kernel_filter(pair<Point, IdealOffset> *d_pairs, Idealinfo *d_info, uint8_t *d_status, PointPixPair* d_batch, uint *d_batch_size, uint size, int *step, bool *resultmap){
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x < size){
		if(resultmap[x] != 0) return;
        pair<Point, IdealOffset> &pair = d_pairs[x];
		IdealOffset &source = pair.second;
		Point &p = pair.first;
		...
		int closest = gpu_get_closest_pixel(p, xoff, yoff, s_dimx, s_dimy);

		if(*step == 0){
			if(gpu_show_status(d_status, source.status_start, closest) == BORDER){
				int idx = atomicAdd(d_batch_size, 1);
				d_batch[idx] = {x, closest};
			}
		}else{
...
__global__ void kernel_refinement(PointPixPair *d_batch, pair<Point, IdealOffset> *d_pairs, uint16_t *d_offset, EdgeSeq *d_edge_sequences, Point *d_vertices, uint *size, double *distance, bool *resultmap){
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x < *size){
		int pair_id = d_batch[x].pair_id;
		int cur = d_batch[x].pix_id;
		...
		for(int i = 0; i < s_num_sequence; ++ i){
			...
			for(int j = 0; j < r.length; j ++){
				double dist = gpu_point_to_segment_distance(p, (d_vertices + source.vertices_start)[r.start+j], (d_vertices + source.vertices_start)[r.start+j+1]);
...
__global__ void kernel_check_exit(pair<Point, IdealOffset> *d_pairs, Idealinfo *d_info, uint8_t *d_status, PointPixPair* d_batch, uint *d_batch_size, uint size, int *step, double *distance, bool *resultmap){
	const int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x < size){
...
uint cuda_within(query_context *gctx){
	...
	PointPixPair *d_batch = nullptr;
    CUDA_SAFE_CALL(cudaMalloc((void **) &d_batch, 64 * 1024 * 1024 * sizeof(PointPixPair)));
	uint h_batch_size;
	...
	while(true){
		...
		kernel_filter<<<grid_size, block_size>>>(d_pairs, gctx->d_info, gctx->d_status, d_batch, d_batch_size, size, d_step, d_resultmap);
		cudaDeviceSynchronize();
		...
		printf("STEP: %d\n", h_step);
		printf("BATCH_SIZE: %u\n", h_batch_size);
		if(h_batch_size == 0) break;	

		grid_size_x = (h_batch_size + BLOCK_SIZE - 1) / BLOCK_SIZE;
		grid_size.x = grid_size_x;

		kernel_refinement<<<grid_size, block_size>>>(d_batch, d_pairs, gctx->d_offset, gctx->d_edge_sequences, gctx->d_vertices, d_batch_size, d_distance, d_resultmap);
		cudaDeviceSynchronize();
		...
		kernel_check_exit<<<grid_size, block_size>>>(d_pairs, gctx->d_info, gctx->d_status, d_batch, d_batch_size, size, d_step, d_distance, d_resultmap);
		cudaDeviceSynchronize();
